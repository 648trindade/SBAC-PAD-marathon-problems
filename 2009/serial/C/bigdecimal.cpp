...
bigdecimal::~bigdecimal() { delete digits; }
...
bigdecimal* bigdecimal::operator+(bigdecimal& decimal)
{
    ...
    while (up >= 0 || down >= 0)
    {
        if (up == down)
        {
            sum = (digits->operator[](dot + up) +
                   decimal.digits->operator[](decimal.dot + down) + ac);
...
bigdecimal* bigdecimal::operator-(bigdecimal& decimal)
{
    ...
    while (up >= 0 || down >= 0)
    {
        ...
        {
            sub =
                (-borrow + 10 - little->digits->operator[](little->dot + down));
            r->digits->push_front(sub % 10);
            borrow = !(sub / 10);
            down--;
        }
...
bool bigdecimal::operator>(bigdecimal& decimal)
{
    ...
    uint i = 0;
    while (digits->operator[](i) == decimal.digits->operator[](i) &&
           i < digits->size() && i < decimal.digits->size())
        i++;
    if (i >= digits->size())
        return false;
...
bool bigdecimal::operator>(bigdecimal* decimal) { return (*this) > (*decimal); }
