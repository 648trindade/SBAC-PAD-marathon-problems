...
{
public:
    comparator(const point& reference) { this->reference = reference; }
    bool operator()(const point& a, const point& b) const
    {
        long int l;
...
void convex_hull(int argc, char* argv[], long unsigned int* h, point* output,
                 const long unsigned int n, const point* input)
{
    if (n == 1)
    {
        *h = 1;
