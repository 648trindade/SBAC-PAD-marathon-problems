...
void knapsack(int argc, char* argv[], long unsigned int* output,
              const long unsigned int capacity, const long unsigned int n,
              const item* items)
{
    vector<const item*> v;
    v.reserve(n);
    ...
    for (vector<const item*>::reverse_iterator it = v.rbegin(); it != v.rend();
         ++it)
    {
        s += (*it)->weight;
        sum.push_back(s);
    ...
    do
    {
        while (i < n)
        {
            if (volume + v[i]->volume <= capacity &&
                weight + sum[i] > bestvalue)
            {
                bitsetA.set(i);
                weight += v[i]->weight;
                volume += v[i]->volume;
